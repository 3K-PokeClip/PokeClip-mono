...
func Load(env func(string) string) (Config, error) {
	...
	// 교차 검증 4 — 폴 주기가 신선도 창 이상이면 관측이 태어나자마자 낡아 ⓐ2 가 영구
	// 불성립이다. 폴은 성공하고 주조만 안 되므로 로그에도 드러나지 않는다.
	if mtx.PollInterval >= obsFresh {
		return Config{}, fmt.Errorf(
			"OBS_POLL(%v)은 OBS_FRESH(%v)보다 짧아야 한다 — 같거나 길면 관측이 언제나 낡아 상태 방증이 성립하지 않는다",
