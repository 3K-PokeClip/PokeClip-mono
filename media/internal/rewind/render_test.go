...
// 수치만 읽는 독립 스크립트(생성기·대조기 전문 — media/README.md 「되감기 M4 ⓑ 이관 기록」 절)가
// 적었고, 설계 4.8.2 실물 발췌의 세 덩어리와는 축자로, 생략된 두 구간(631·264조각)과는 줄 수로
// 대조했다. 단 VERSION 줄만 kty 결정으로 6 이다(설계 4.8.2 의 9 에 대체 표식 · 계획 부기 32). 나머지
// 기대값은 손으로 적은 리터럴이다.
...
//
// 외부 테스트 패키지로 두는 이유: 소비자(ⓒ 발행 · 축출 증분 EvictedDiscontinuityTags)가 쓰는 공개
// 계약만으로 성립하는지가 곧 렌더의 계약 검증이다.
...
// 술어에 목록 소유 회차를 넘기면 S 목록의 seq 10 표시가 빠지고, 축출 증분(EvictedDiscontinuityTags —
// 행의 회차로 센다)이 나간 표시 없이 DISC-SEQ 를 올린다.
