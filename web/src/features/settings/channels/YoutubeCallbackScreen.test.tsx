...
import { youtubeLinkQueryOptions } from '@/api/youtubeLink';
...
/** POST /api/youtube-link 호출만 센다 — 1회용 code가 두 번 나가지 않는지가 요점이다. */
const exchangeCalls = (spy: ReturnType<typeof stubFetch>) =>
  spy.mock.calls.filter(([url, init]) => url === '/api/youtube-link' && init?.method === 'POST');
...
describe('YoutubeCallbackScreen', () => {
  it('교환에 성공하면 캐시를 심지 않고 성공 토스트를 띄운 뒤 채널 화면으로 되돌아간다', async () => {
    stubFetch(() =>
      jsonResponse(201, {
    ...
    const queryClient = renderScreen(<YoutubeCallbackScreen />);

    await waitFor(() => expect(replace).toHaveBeenCalledWith('/settings/channels'));
    expect(await screen.findByText('유튜브 채널을 연동했어요')).toBeInTheDocument();

    // 캐시에 계정 데이터를 심지 않는다 — 다른 탭이 계정을 바꾼 뒤 늦게 도착한 응답이
    // 이전 계정 채널을 되살리지 않게, 비워서 목적지가 처음부터 읽게 한다
    expect(queryClient.getQueryData(youtubeLinkQueryOptions.queryKey)).toBeUndefined();
    expect(document.body.innerHTML).not.toContain('yt-secret-1');
  });
