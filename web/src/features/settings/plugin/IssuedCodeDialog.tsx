...
// 열려 있는 동안 10분 카운트다운이 흐르고, 만료되면 코드를 숨기고 재발급을 안내한다.
        ...
        {countdown.expired ? (
          <>
            ...
            <p role="status" className={styles.issueExpired}>
              코드가 만료됐어요 — 발급 후 10분이 지났어요. 새 코드를 발급해 주세요.
            </p>
            <div className={styles.confirmActions}>
              <Button variant="ghost" size="sm" onClick={onClose}>
                닫기
              </Button>
              <Button variant="solid" size="sm" loading={busy} onClick={onIssueNew}>
                새 코드 발급
              </Button>
            </div>
          </>
          ...
          <>
            ...
            <div className={styles.issueCodeRow}>
              ...
              <Button variant="soft" size="md" onClick={() => void copy()}>
                {copied ? '복사됨' : '복사'}
              </Button>
            </div>
            ...
            <div className={styles.issueWarning}>
              이 코드는 지금 <b>한 번만</b> 표시돼요. 창을 닫으면 다시 확인할 수 없고, 잃어버리면
              재발급해야 합니다.
            </div>
