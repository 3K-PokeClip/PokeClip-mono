...
| `BROADCAST_REAPER_GRACE` | `PT30M` | 마지막 조각 뒤 이만큼 조용하면 닫는다. **15분 미만은 부팅 거부**(계약9 정산 창) |
...
**유예(`BROADCAST_REAPER_GRACE`) 기본 30분, 15분 미만은 부팅 거부.** 송출이 끊겨도 1번은 슬레이트를
5분 이어 조각을 만들고(계약-세그먼트인덱스 6-5), 종료 편지는 그 뒤 정산 창 **15분**이 지나야 발행한다
(계약9 게이트 ④). 즉 마지막 조각 뒤 15분 안에는 **정상 편지가 아직 오는 중**이다. 유예가 그보다 짧으면 이
장치가 편지보다 먼저 닫아, 편지가 정상인데도 매 방송이 두 번 닫힌다. 30분은 거기에 여유 10분이다.
편지가 정상이면 **이 장치는 한 번도 안 돈다.** 🔴 값은 착수 때 정한 제안이지 사용자 확정이 아니다(카드 TBD).
