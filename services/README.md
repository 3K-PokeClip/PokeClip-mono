...
| `CHAT_DETACH_ENABLED` | `false` | ✅ | 켜면 재부착 회차에서 **내가 붙어 있는데 clip 명부에 없는 방송**의 세션을 반납한다(POK-244). clip 치우개(`BROADCAST_REAPER_ENABLED`)와 같이 켠다 — clip이 놓친 방송을 닫으면 다음 회차(1분)에 자리가 돌아온다. 명부가 잘린(`truncated`) 회차는 통째로 건너뛴다 |
...
| `StreamKeyService.ensureKey` — 확인이 **락 없이** 앞서고 키 생성이 `REQUIRES_NEW`다 | 수십 ms | 살아있는 스트림키 하나. `resolve`가 그 키에 `valid:true`를 준다 |
...
🔴 **왜 안 닫나 — 완전히 닫으려면 「이미 회원 행 락을 잡는 경로」여야 하는데, 페어링 교환은 그러면 안 된다.**
락을 안 잡던 경로에 새로 얹으면 **잠금 순서가 생긴다**: 교환은 `pairing_codes` 행을 먼저 잠그고
탈퇴는 회원 행을 먼저 잠근다 — **순서가 반대라 사이클**이다. 게다가 스트림키 생성이 `REQUIRES_NEW`라
...
- **페어링 교환이 회원 행 락을 잡게 되면** — 위 사이클이 사라져 셋 다 `requireAliveWithLock`으로 닫을 수 있다
...
**안 떼는 조건 셋**이 이 갈래의 전부다: ① 명부가 상한 500에 **잘렸다**(`truncated`) — 살아있는 방송이 「사라진 것」으로
보이므로 그 회차는 통째로 건너뛰고 `chat.reattach.detach_skipped reason=TRUNCATED`를 남긴다 ② 방송 시작 뒤 유예
(`CHAT_DETACH_GRACE`, 기본 10분) 안이다 — 같은 편지를 clip과 각자 받으므로 우리가 먼저 붙고 clip이 아직 안 적은 순간이
있다 ③ 꺼져 있다. **메모(`chat_ended_streams`)는 안 남긴다** — 메모의 열쇠는 편지의 순서 번호이고 여기엔 편지가 없다.
재전송된 시작 편지가 그 사이 세션을 다시 열면 다음 회차가 다시 뗀다. 로그 `chat.reattach.detached stream= startedAt=` ·
`chat.reattach.detach_swept detached= failed=`. 시험은 실물 등록부에 세션을 세우고 반납이 등록부에서 실제로 사라지는
것까지 잰다(주입: 반납 호출을 지우면 빨간불).
