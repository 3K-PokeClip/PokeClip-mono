...
| `POST /api/chzzk-link` `{code, state}` | 201 `{channelId, channelName, linkedAt}` · 400 `INVALID_STATE` · 400 `INVALID_CODE`(치지직이 교환·me를 4xx로 거부 — code 소모·만료·scope 부족, **동의부터 다시**) · 409 `CHANNEL_ALREADY_LINKED` · 502 `CHZZK_UNAVAILABLE`(5xx·타임아웃·429·408 — 잠시 후 재시도) |
...
(`revoke_reason=REFRESH_REJECTED` → `BROKEN`, 재시도 없음 — 429·408은 일시로 보고 재시도) · 5xx·타임아웃이면
...
치지직에 살아있을 수 있음)·`token_already_dead`(INFO — 4xx, 이미 무효. 429·408은 제외 — 그 둘은 Unavailable →
WARN `orphan_token causeType=Http429/408`)·`resolve_rejected`·`failed`).
