...
**400은 사유 낱말 하나다** — `from`·`to`·`too_wide`(창 1시간 초과)·`inverted_window`·`limit`·
`kinds`·`bucket`·`cursor`·`too_many_buckets`. **`limit`은 상한(500)을 넘기면 400이 아니라 잘라 준다**
...
🔴 **후원에도 지문 UNIQUE가 있다**(`uq_chat_donations_fingerprint`, POK-234 로컬 리뷰).
`(stream_id, donator_channel_id, received_at, donation_sha256)`이고 저장 재시도가 만드는
중복을 `ON CONFLICT DO NOTHING`이 흡수한다 — 배치 저장이 절단되면 커밋된 앞 행이 되돌려져
다시 들어오는데, 채팅은 이 제약이 흡수하는 반면 후원에는 흡수 장치가 없었다.
**「같은 사람이 같은 금액을 두 번」은 안 접힌다** — 지문에 **시각이 들어 있어서**다.
접히는 것은 「같은 사람이 **같은 밀리초에** 같은 내용으로」뿐이고, 후원은 결제 왕복을
거치므로 그것은 재시도 말고 일어나지 않는다. 접힌 수는 `DonationPersister.conflictedCount()`가
따로 센다 — **유실이 아니라 중복이다.**
