...
과거로 오염시키므로 버리는 편이 낫다). **예외는 하나뿐이고 아래 「알려진 구멍」 ②-3에 있다.**
...
**이 서버는 창구가 없어 로그가 유일한 관측 통로다.** 열여섯 줄이고, 접두어는 전부 `detect.`다.

| 이벤트 | 레벨 | 언제 보나 |
|---|---|---|
| `detect.card_published` | INFO | **카드가 나갈 때마다.** 지연 두 숫자가 여기 있다 — 아래 절 |
| `detect.card_skipped` | INFO | 판정은 급증인데 안 보냈다. **`reason=`이 처분을 가른다** — `NOT_YET_INDEXED`·`UNAVAILABLE`은 **발행권을 되돌려 되돌아보기 안에서 다시 시도**하고, `NO_FOOTAGE`(영영 없다)·`invalid_window`(위치가 음수 등)는 그대로 포기한다 |
...
| `detect.publish_rejected` | WARN | clip이 4xx를 줬다. **재시도하지 않는다** — 같은 본문이라 영영 같은 답이다 |
...
되돌리지 **않는** 것: `no_footage` · 위치가 음수 · clip이 4xx로 거절 · 시도 소진 ·
**`UNAVAILABLE`(창구를 못 물었다)**. 다섯 다 다시 물어도 답이 같거나, 되물어 봐야 값이 없다.
...
지금 상한(발행권을 잡은 시각)은 그 결정과 무관하게 늘 비관 쪽이다.
