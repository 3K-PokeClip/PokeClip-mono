...
**종료가 구독 반납 왕복을 기다린다.** 반납 도중에 인터럽트하면 세션 키는 이미
소모된 뒤라 **아무도 다시 못 보내고, 스트리머당 3개뿐인 자리가 하나 남는다.**
치지직이 느린 날 재시작을 반복하면 그대로 마른다.

최악 대기가 **9초**이고 그 뒤 마지막 정리가 반납을 한 번 더 보낼 수 있다.
**도커 기본 유예는 10초라 모자란다.**
