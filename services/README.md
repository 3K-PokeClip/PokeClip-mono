...
| `CHAT_SYNC_OFFSET_MS` | `0` | — | 채팅 시각에서 빼는 보정값(ms). **음수를 허용하고 크기는 ±600000(10분)까지다.** 실측으로 정한다 — 아래 참고 |
...
이 그물은 **자릿수 착각만 자르고 실측값은 통과시킨다.** 상한을 10분에 둔 근거는 이 값이
무엇의 합인지다 — 지배 항인 방송 지연·반응 지연이 **초 단위**이므로 합의 현실적 크기는
수 초~수십 초이고, 10분은 거기에 10배 이상의 여유를 준 자리다. 그러면서 초를 마이크로초로
착각한 값(1.5초 → 25분)도 나노초로 착각한 값(1.5초 → 17일)도 걸린다.
...
```json
{"streamId":"live-A-001","state":"converted","positionMs":5500,"segmentSeq":2,"appliedOffsetMs":0}
```
...
`services/docker-compose.dev.yml`에는 그것이 없으므로 **dev 환경에서 이 창구는 500이다.**
dev에서 500을 보면 장애가 아니라 이것부터 본다.
