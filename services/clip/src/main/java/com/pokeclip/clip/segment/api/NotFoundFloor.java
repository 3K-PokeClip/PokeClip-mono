...
/**
 * 세그먼트 창 조회의 404를 <b>정해진 시각 전에는 내보내지 않는</b> 자리.
 *
 * <p><b>왜 있나.</b> 이 문의 404는 두 갈래다 — 「없는 방송」은 명부 조회 하나로 끝나고,
 ...
 *       {@code JumpCardExceptionHandler}의 404도 범위 밖이다(그쪽은 갈래가 하나뿐이라 맞출 짝이 없다)</li>
 * </ul>
 */
final class NotFoundFloor {

    /** 이 문의 404는 기준 시각으로부터 이보다 빨리 나가지 않는다. 값의 근거는 클래스 주석. */
    static final Duration FLOOR = Duration.ofMillis(25);
    ...
    static void mark(HttpServletRequest request) {
     ...
     *
     * <p>🔴 <b>인터럽트를 삼키지 않는다.</b> 삼키면 종료 중에 들어온 요청이 스레드 종료를 막는다.
     * 깃발을 되살리고 즉시 나가므로 <b>그 요청 하나는 바닥을 못 채운다</b> — 서버가 내려가는
     * 중에만 벌어지는 일이라 그대로 둔다.
     */
    static void awaitFloor(HttpServletRequest request) {
        long remainingNanos = request.getAttribute(START_NANOS) instanceof Long startNanos
