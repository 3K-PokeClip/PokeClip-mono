 ...
 *
 * <p>🔴 <b>정확히는 「<u>이기는 쪽</u> 봉투의 차이」다 — 그리고 지는 쪽이 더 위험하다.</b>
 * 감사가 반대 방향도 재 봤다: 좁힌 이 조언에 같은 타입을 되돌리되 본문을 {@code SCOPED-WINS}로
 * 바꿔 돌리자 <b>27건이 전부 초록</b>이었다(지는 쪽 본문은 한 번도 안 나갔다).
 * 즉 <b>지는 쪽은 코드에 멀쩡히 살아 있는데 무엇을 고쳐도 시험이 안 변한다</b> —
 * 고치는 사람은 자기 수정이 반영되는지를 확인할 방법이 없고 <b>초록을 「고쳐졌다」로 읽는다.</b>
 * 이 조언에 무언가를 더할 때 전역 쪽에 같은 타입이 있는지부터 봐라.
 *
