 ...
 *
 * <p>본문을 {@code String}으로 받는 이유는 {@link RecipeParser} 주석에 있다 — 파싱 실패까지 이 문 안에서
 * 400 {@code invalid_request} 봉투로 끝내려는 것이다. {@code consumes}로 JSON만 받으므로 다른 형은 415다.
 */
@RestController
...
public class RecipeController {
    ...
    public ResponseEntity<RecipeSnapshot> create(@PathVariable String streamId,
        ...
        // 파싱이 mark보다 앞인 것은 의도다 — 파싱 실패(400)는 방송의 존재와 무관하게 나가므로 감출 것이 없다.
        // 규칙 검증(400)은 서비스 안에서 자격 판정 뒤에 돈다(RecipeService 주석).
        NotFoundFloor.mark(request);
        RecipeSnapshot created = service.create(jwt.getSubject(), streamId, parser.parse(body));
    ...
    public RecipeSnapshot replace(@PathVariable String streamId,
                                  ...
                                  HttpServletRequest request) {
        NotFoundFloor.mark(request);
        return service.replace(jwt.getSubject(), streamId, id, parser.parse(body));
    }
}
