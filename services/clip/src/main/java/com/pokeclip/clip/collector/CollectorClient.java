 ...
 *
 * <p><b>4xx는 예외가 아니다.</b> 수집기의 400 본문은 {@code {"error":"inverted_window"}}처럼
...
public class CollectorClient {
    ...
    /** 수집기의 {@code InternalTokenFilter}가 보는 이름. 빠지면 401이고, 401은 4xx라 그대로 나간다. */
    ...
    public CollectorResponse get(String path, Map<String, String> query) {
        ...
        try {
            ResponseEntity<String> response = restClient.get()
                    ...
                    .retrieve()
                    // 4xx를 예외로 안 바꾼다. 기본 핸들러를 비우는 것이 그 뜻이다.
            ...
            // 5xx만 여기로 온다(4xx는 위에서 삼켰다). 본문·예외 메시지를 안 찍는다 —
