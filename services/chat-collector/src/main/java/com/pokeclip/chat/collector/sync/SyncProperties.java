...
public record SyncProperties(long defaultOffsetMs, Map<String, Long> channelOffsetMs) {
     ...
     *
     * <p><b>🔴 이 그물은 위쪽만 자른다.</b> 밀리초를 마이크로초로 착각한 값(1.5초 → 25분)도
     * 나노초로 착각한 값(1.5초 → 17일)도 상한을 넘어 걸리지만, <b>반대 방향은 안 걸린다</b> —
     * 3.9초를 {@code 3900} 대신 {@code 4}(초 단위)로 적으면 4ms가 그대로 통과하고 서버는 멀쩡히 뜬다.
     *
     * <p><b>하한은 둘 수 없다.</b> {@code 0}이 「보정 안 함」이라는 유효한 값이고 실제로 이 설정의
     * 자리 표시 상태이기 때문이다. 그래서 작은 쪽 착각은 <b>구조적으로 못 막는다</b> — 결말이
     * 가벼운 것도 이유다(위치가 그만큼 덜 정확할 뿐, 큰 쪽 착각처럼 {@code no_footage}로 가서
     * 방송 전체의 하이라이트를 잃지 않는다). 다만 <b>0으로의 되돌림</b> 하나는
     * {@code VideoPositionCalculatorTest.실물_기본_보정값이_자리_표시_0이_아니다()}가 잡는다.
     *
     * <p>{@code VideoPositionController}의 1970~2200 그물이 입력 쪽에서 큰 쪽 착각을 같이 자른다.
     */
