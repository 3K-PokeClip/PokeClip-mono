...
public final class StreamerSerialExecutor implements AutoCloseable {
     ...
     *
     * <p><b>대기까지 버리는 이유</b>: 거절은 실행기가 닫혔다는 뜻이고, 그러면 <b>이 줄에 새로
     * 시작되는 것이 없다.</b> (🔴 예전에는 「이 줄의 <b>무엇도</b> 다시 돌 수 없다」고 적었는데
     * <b>틀렸다</b> — 감사가 반례를 짚었다: 이미 돌고 있는 작업이 있으면 새 작업은
     * {@code pending}에 붙기만 하고 {@code execute}를 안 타므로 거절이 나지 않고,
     * {@code run} 루프의 {@code nextOrRelease}가 그것을 꺼내 <b>그대로 실행한다.</b>
     * 동작은 그쪽이 더 안전하다 — 작업이 돌아 알림이 지워지거나, 못 지우면 큐에 남는다.
     * 고칠 것은 코드가 아니라 이 문장이었다.) 남겨 두면 그 작업들도 영영 안 돌면서 {@code inFlight}만 붙들어
