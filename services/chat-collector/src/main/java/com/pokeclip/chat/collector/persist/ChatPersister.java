...
public class ChatPersister implements PersistCounters {
     ...
     * 이미 진행 중)이어도 둘째 대기가 같은 5초라 합산이 같다. 반납 REST 최악
     * (접속 2초+읽기 5초)까지 겹치면 더 늘 수 있어, "최악 합산 ≤ 유예 20초 −
     * 여유 2초"에 들도록 값들을 잡았다 — 유예 권고는 services/README.md에 있다.
     */
    @PreDestroy
    public void close() {
        if (!closed.compareAndSet(false, true)) {
            try {
                ...
                // stop 앞단 최악 9초 + 이 대기 + 뒷정리. 5초면 최악 합산이
                // 14~15초 + flush 실행분으로 유예 20초 − 여유 2초 안에 든다.
            ...
            if (buffer.size() > 0 && scheduler.isTerminated()) {
        ...
        try {
            ...
            // 14초 + flush 실행분 — 운영 종료 유예(15초 이상 요청해 둠) 안에 든다.
    ...
    public int flushOnce() {
        ...
        try {
            ...
            // reWriteBatchedInserts를 켜면 드라이버가 행별 카운트 대신 SUCCESS_NO_INFO(-2)를
            // 준다 — 그대로 합치면 persisted가 음수로 무너진다. 행별 정보가 없으면
            // 충돌을 못 가르므로 전량 저장으로 계상한다(지금 JDBC url에는 그 옵션이
            // 없어 실측 카운트가 온다 — 이 가드는 옵션이 켜지는 날의 방어선이다).
            int saved = IntStream.of(results).anyMatch(c -> c < 0)
                    ? batch.size()
                    : IntStream.of(results).sum();
            recordOutcome(batch.size(), saved);
            return batch.size();
