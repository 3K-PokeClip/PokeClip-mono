 ...
 * ① 그 채널에 다음 분 채팅이 오면 이전 창은 즉시 닫는다(그 뒤로 이전 분 채팅이 올 일이
 *    없다 — 수신 순서 = 받은 시각 순서다) ② 채팅이 안 와도 틱 시각이 창 끝 + 유예(2초 =
...
public final class MinuteBatcher {
    ...
    private static final class OpenWindow {
        ...
        ArchiveObject toObject(String runId) {
            return new ArchiveObject(ArchiveKey.of(channelId, windowStart, runId, seq), bytes.toByteArray(), count, channelId, windowStart);
        }
    }
    ...
    public List<ArchiveObject> accept(List<ArchivableChat> drained, long nowMillis) {
        ...
        for (ArchivableChat chat : drained) {
            ...
            try {
                ...
                if (encodeFailures.incrementAndGet() == 1) {
            ...
            if (window != null && window.windowStart != start) {
                closeWindow(window, closed);              // ① 다음 분이 왔다 — 이전 창 닫기
                window = null;
            }
            if (window == null) {
     ...
     * (리뷰 1회차 사소 2, 사용자 결정 2026-08-16). 재열림 횟수는 reopened로 센다.
     */
    ...
    /** 닫으면서 그 열쇠의 순번을 기억한다 — 다음 재열림이 다음 순번을 받게. open 맵에서 빼는 것은 호출자 몫. */
    ...
    /** 오래된 열쇠는 잊는다 — 방송 3시간이면 채널당 180개가 쌓이는데 재열림 창은 유예 몇 초라 2시간이면 충분하다. */
    private void forgetOldClosed(long nowMillis) {
        long cutoff = nowMillis - CLOSED_MEMORY_MS;
        for (Map<Long, Integer> perChannel : closedSeq.values()) {
            perChannel.keySet().removeIf(start -> start < cutoff);
        }
    }

    /** 같은 창이 다시 열린 횟수 — 0이 정상이다. 관측용. */
