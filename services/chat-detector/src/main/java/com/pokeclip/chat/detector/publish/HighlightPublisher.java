...
public class HighlightPublisher {
    ...
    public Outcome publish(String streamId, long metricId, long windowStartMs,
        ...
        // 🔴 404는 나머지 4xx 와 처분이 반대다(봇 리뷰 1판). clip 이 그 방송을 아직 모르는
        // 것뿐이고, 방송 시작 알림을 수집기와 clip 이 각자 다른 큐에서 받으므로 clip 이 늦으면
        // 정상 운영에서 온다. 영구 실패로 접으면 그 하이라이트가 영영 사라진다.
        // 거절(다른 4xx)과 시도 소진은 그대로 포기한다 — PRD 결정이고, 다시 보내도 같거나
        // 이미 늦어 되감기 창 밖이다.
        Outcome outcome = switch (result) {
            case CREATED, ALREADY_EXISTS -> Outcome.SENT;
            case BROADCAST_NOT_FOUND -> Outcome.RETRY_LATER;
            case REJECTED, FAILED -> Outcome.GIVE_UP;
        };

        // 🔴 「나갔다」는 줄은 실제로 나갔을 때만 찍는다(봇 리뷰 1판, codex). 실패에도 찍으면
        // 이벤트 이름으로 세는 쪽이 배달된 하이라이트를 과대 집계하고, 실패 지연이 성공 분포에
    ...
    private void logLatency(String streamId, HighlightCard card, long windowStartMs, long windowSizeMs,
        ...
        // 🔴 count 는 <b>판정에 실제로 쓴 지표</b>의 값이다(봇 리뷰 1판, codex). 늘 messageCount 를
