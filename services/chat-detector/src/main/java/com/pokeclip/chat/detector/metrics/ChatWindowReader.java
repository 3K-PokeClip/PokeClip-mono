...
public class ChatWindowReader {
     ...
     * 실측: 버퍼 124 → 42, 결과 행은 같다 — <b>이 값은 2026-09-03 감사 라운드 1의 관측이고
     * 그 뒤 다시 재지 않았다</b>. 이 서버에는 {@code EXPLAIN} 검사가 0개라 계획이 바뀌어도
     * 시험이 안 잡는다). <b>두 칸을 다 거는 것은 그대로 옳다</b> — 창 경계는
     ...
     *
     * <p><b>이 값은 2026-09-03 감사 라운드 1의 관측이고 그 뒤 다시 재지 않았다.</b>
     * 짚은 것은 「{@code V306}이 그 뒤 한 글자도 안 바뀌었다」는 <b>전제</b>뿐이고, 계획 자체를
     * 다시 잰 것이 아니다. 이 서버에는 {@code EXPLAIN} 검사가 <b>0개</b>라 계획이 바뀌어도
     * 시험이 안 잡는다(수집기 쪽은 {@code ChatWindowQueryTest}가 운영 상수를 직접 EXPLAIN 한다).
     *
