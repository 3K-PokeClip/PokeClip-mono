...
public class ChatMetricsStore {
    ...
    private static final String CLAIM = """
            ...
            RETURNING id, first_claimed_at
            """;

    /**
     * 🔴 <b>{@code COALESCE}가 이 문장의 요점이다.</b> 되돌렸다 다시 집는 창에서
     * {@code first_claimed_at}이 <b>첫 값 그대로</b> 남아야 한다.
     *
     * <p>그 값이 지연을 잴 때 「집계에 쓰인 채팅」의 상한이 된다. 재시도마다 새 시각으로
     * 덮으면 상한이 뒤로 밀리고, <b>판정에 쓰이지도 않은</b> 늦은 채팅이 {@code max(received_at)}에
     * 섞여 우리 구간이 <b>실제보다 짧게</b> 나온다 — 감사 2회차 R-2가 고친 병과 같은 것이
     * 재시도 경로로 되살아난 것이다(봇 리뷰 2판, codex).
     *
     * <p><b>낙관 방향이라는 것이 이 병의 핵심이다</b> — 목표 3초를 재는 숫자가 우리를
     * 좋게 보이게 한다.
     *
     * @return 잡았으면 그 줄의 번호와 <b>처음</b> 집은 시각. 이미 누가 잡았으면 빈 값
     */
    public Optional<Claim> claimForPublish(String streamId, long windowSizeMs, long windowStartMs, Instant now) {
        List<Claim> claims = jdbc.query(CLAIM,
                (rs, n) -> new Claim(rs.getLong("id"), rs.getTimestamp("first_claimed_at").toInstant()),
                Timestamp.from(now), Timestamp.from(now), streamId, windowSizeMs, windowStartMs);
        return claims.isEmpty() ? Optional.empty() : Optional.of(claims.get(0));
    }

    /**
     * 발행권 하나. <b>번호와 「처음 집은 시각」이 같이 다닌다</b> — 부르는 쪽이 둘을 따로
     * 구하면 재시도 판에서 어긋난다.
     *
     * @param firstClaimedAt 이 창을 <b>처음</b> 집은 시각. 되돌렸다 다시 집어도 안 바뀐다
     */
    public record Claim(long metricId, Instant firstClaimedAt) {
    }
