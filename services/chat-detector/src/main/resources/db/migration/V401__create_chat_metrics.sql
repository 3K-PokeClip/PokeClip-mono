...
CREATE TABLE chat_metrics (
    ...
    -- 🔴 이 창을 <b>처음</b> 집은 시각. 되돌렸다 다시 집어도 <b>안 바뀐다</b>.
    -- 지연을 잴 때 「집계에 쓰인 채팅」의 상한으로 쓴다 — published_at 을 쓰면 재시도할 때마다
    -- 상한이 뒤로 밀려, 판정에 쓰이지도 않은 늦은 채팅이 max(received_at) 에 섞이고
    -- 우리 구간이 실제보다 짧게(=낙관적으로) 나온다.
    --
    -- created_at 을 대신 쓰지 않는 이유: PostgreSQL now() 는 문장 시각이 아니라 트랜잭션 시작
    -- 시각이라, 그 값의 위치가 트랜잭션 경계라는 무관해 보이는 결정에 딸려 움직인다
    -- (실측: 오토커밋에서 집계 조회보다 +2.003초 뒤 = 낙관 쪽).
