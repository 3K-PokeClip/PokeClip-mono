...
class ChzzkCleanupExecutorTest {
    ...
    void 큐가_차면_거부를_WARN으로_남기고_awaitIdle은_끝난다() throws Exception {
        ChzzkCleanupExecutor executor = new ChzzkCleanupExecutor();
        try {
            ...
            try (LogCaptor logs = new LogCaptor()) {
                try {
                    ...
                    for (int i = 0; i < ChzzkCleanupExecutor.THREADS + ChzzkCleanupExecutor.QUEUE_CAPACITY; i++) {
                        executor.submit(executor.new Job(1L, null, blocking));
                    }
                    executor.submit(executor.new Job(99L, null, ran::incrementAndGet));
                    assertThat(logs.messages()).anyMatch(m -> m.equals("auth.chzzk.cleanup.rejected userId=99 reason=queue_full"));
                    assertThat(logs.levelOf("auth.chzzk.cleanup.rejected")).isEqualTo(Level.WARN);
                ...
                assertThat(ran.get()).isEqualTo(ChzzkCleanupExecutor.THREADS + ChzzkCleanupExecutor.QUEUE_CAPACITY);
            }
        } finally {
            executor.shutdown();
        }
