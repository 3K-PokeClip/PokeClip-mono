...
class ChzzkLinkStartAndLinkTest extends ChzzkLinkTestSupport {
    ...
    /** 교환 응답에 토큰은 있는데 expiresIn이 깨졌다 — 토큰은 이미 발급됐으니 502를 내더라도 버린다(부분 파싱 토큰). */
    @Test
    void 교환_응답의_토큰은_읽혔는데_형식이_깨지면_502이고_읽힌_토큰은_버린다() throws Exception {
        User user = newUser();
        CHZZK.tokenResponds(200, "{\"code\":200,\"content\":{\"accessToken\":\"at-x\",\"refreshToken\":\"rt-x\",\"expiresIn\":true}}");
        mockMvc.perform(link(user)).andExpect(status().isBadGateway())
                .andExpect(jsonPath("$.reason").value("CHZZK_UNAVAILABLE"));
        assertThat(CHZZK.revokedTokens()).containsExactlyInAnyOrder("at-x", "rt-x");
        assertThat(linkRepository.findFirstByUserIdOrderByCreatedAtDesc(user.getId())).isEmpty();
    }
