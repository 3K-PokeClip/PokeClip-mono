...
class YoutubeCleanupExecutorTest {
    ...
    void 종료에_잘려_버려진_잡은_회원_번호를_남긴다() throws Exception {
        YoutubeCleanupExecutor executor = new YoutubeCleanupExecutor();
        CountDownLatch 워커가_잡혔다 = new CountDownLatch(YoutubeCleanupExecutor.THREADS);
        ...
        try (LogCaptor logs = new LogCaptor()) {
            try {
                ...
                assertThat(logs.messages())
                        ...
                        .contains("auth.youtube.cleanup.dropped userId=11",
                                "auth.youtube.cleanup.dropped userId=22",
                                "auth.youtube.cleanup.dropped userId=33");
                assertThat(logs.levelOf("auth.youtube.cleanup.dropped")).isEqualTo(Level.WARN);
                assertThat(logs.messages())
                        .as("돌던 것과 안 돈 것을 뭉치면 「몇 명이 복구 불가인가」를 못 말한다")
                        .anyMatch(m -> m.startsWith("auth.youtube.cleanup.shutdown_timeout dropped=3 "));
            } finally {
                놓아준다.countDown();
            }
        }
