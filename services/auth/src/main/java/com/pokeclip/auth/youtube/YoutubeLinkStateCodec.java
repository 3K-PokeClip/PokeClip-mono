...
/**
 * {@code chzzk/ChzzkLinkStateCodec}의 복제다 — 한쪽을 고치면 나란히 놓고 비교한다.
 * 다른 점은 도메인 접두어와 TTL을 읽어 오는 프로퍼티뿐이다.
 *
 * <p>공용화하지 않은 이유: 공용화하려면 실측·감사로 굳은 치지직 파일을 옮겨야 하고(패키지·이름·로그 이벤트),
 * 그 회귀 위험이 복제 유지비보다 크다고 봤다(계획 2절 결정 1).
 *
 * <p>동의 왕복의 state를 표 없이 서명으로 만든다.
 *
 ...
 * 그대로 주입받는다. 도메인 분리 접두어 "youtube-link-state:"를 서명 대상에 붙여, <b>같은 키를 쓰는</b>
 * 치지직 state를 여기 내도 검증이 안 된다(YoutubeLinkStateCodecTest가 양방향으로 못박는다).
 *
 ...
 */
@Component
public class YoutubeLinkStateCodec {

    private static final String DOMAIN = "youtube-link-state:";
    ...
    @Autowired
    public YoutubeLinkStateCodec(SecretKey jwtSecretKey, YoutubeProperties youtube) {
        this(jwtSecretKey, youtube.stateTtl());
    }

    YoutubeLinkStateCodec(SecretKey key, Duration ttl) {
        this.key = key;
        this.ttl = ttl;
    }
