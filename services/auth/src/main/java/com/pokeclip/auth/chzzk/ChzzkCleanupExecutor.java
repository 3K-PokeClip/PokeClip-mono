...
/**
 * 커밋 뒤 정리(옛 secrets 삭제 2 · 순서 로그 · 옛 토큰 revoke 2)를 <b>요청 스레드가 아닌 전용 스레드</b>에서 돈다.
 *
 ...
 * 쥔 채 두 번째 커넥션을 요구해, 풀 크기(운영 10)만큼의 동시 4xx 갱신·해제·재연동에서 풀 데드락이 됐다
 * (감사 3회차 실측: 25건 중 21건 500, 30초 마비, 고아 secrets 42). afterCommit 안에서는 <b>제출만</b> 한다.
 *
 * <p>대가와 그 처리 — ① 큐가 차면 조용히 사라지지 않고 {@code auth.chzzk.cleanup.rejected} WARN(고아 행의 원인을
 * 찾을 수 있게) ② SIGTERM에 즉시 죽으면 대기 중 삭제가 유실되므로 {@code @PreDestroy}에서 짧게 기다린다
 * ③ 뒤처리에 치지직 revoke(외부 HTTP 최대 5초)가 들어가므로 스레드 2개 ④ 그 스레드엔 MDC가 없다 —
 * requestId를 값으로 들고 가서 다시 넣는다. 프로세스가 죽으면 고아 secret이 남는 것은 전과 같다.
 */
@Component
public class ChzzkCleanupExecutor {

    private static final Logger log = LoggerFactory.getLogger(ChzzkCleanupExecutor.class);
    ...
    /**
     * 현재 트랜잭션이 커밋되면 정리(delete·revoke) action을 전용 스레드에 제출한다. 트랜잭션이 없으면
     * IllegalStateException — 그게 맞다(자기 호출로 프록시를 안 탄 것을 잡아 준다).
     *
     * <p>로그의 자리 — {@code refreshed}만 요청 스레드 동기 afterCommit(ChzzkTokenRefresher.logAfterCommit)이다.
     * {@code relinked}·{@code unlinked}·{@code refresh_rejected}는 정리 잡 안에서 secrets 삭제 뒤에 찍힌다 —
     * "정리까지 끝났다"는 순서 로그다. 큐가 거부되면 그 로그도 함께 사라지고, 그때 {@code cleanup.rejected} WARN이
     * 신호다. requestId는 잡이 값으로 옮긴다.
     */
    ...
    void shutdown() {
        ...
        try {
            if (!pool.awaitTermination(SHUTDOWN_WAIT.toMillis(), TimeUnit.MILLISECONDS)) {
                log.warn("auth.chzzk.cleanup.shutdown_timeout pending={}", pool.getQueue().size() + pool.getActiveCount());
            }
        } catch (InterruptedException e) {
            Thread.currentThread().interrupt();
        }
    ...
    final class Job implements Runnable {
        ...
        public void run() {
            ...
            try {
                ...
                log.warn("auth.chzzk.cleanup.failed userId={} causeType={}", userId, e.getClass().getSimpleName());
            } finally {
