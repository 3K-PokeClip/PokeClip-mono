    ...
    void shutdown() {
        ...
        try {
            if (!pool.awaitTermination(SHUTDOWN_WAIT.toMillis(), TimeUnit.MILLISECONDS)) {
                ...
                log.warn("auth.withdrawal.cleanup.shutdown_timeout dropped={} interrupted={}",
                        dropped.size(), interrupted);
                for (Runnable job : dropped) {
                    log.warn("auth.withdrawal.cleanup.dropped userId={}", job instanceof Job j ? j.userId : null);
                }
                if (!pool.awaitTermination(FORCED_STOP_WAIT.toMillis(), TimeUnit.MILLISECONDS)) {
                    log.warn("auth.withdrawal.cleanup.shutdown_forced dropped={} interrupted={}",
                            dropped.size(), interrupted);
                }
            }
        } catch (InterruptedException e) {
