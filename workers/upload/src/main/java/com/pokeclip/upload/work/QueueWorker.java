...
/**
 * 줄에서 한 번에 <b>한 통</b>만 꺼내 처리한다. 업로드는 망 대역을 다 쓰는 일이고, 유튜브 하루 한도(약 5개)가 있어 겹쳐 돌려 얻을 것이 없다.
 *
 * <p>처리하는 동안 숨김 시간을 늘린다: 남은 시간이 2분 아래로 내려가면 1분마다 2분으로 다시 잡는다. 안 늘리면 긴 업로드 도중
 * 쪽지가 다시 보여 다른 일꾼이 같은 주문을 잡는다. 그래도 영상은 하나다(바이트는 clip에 적힌 주소 하나로만 간다) — 늘리는 것은
 * 대역을 두 번 쓰지 않기 위해서다.
 */
public class QueueWorker implements SmartLifecycle {
    ...
    private final ScheduledExecutorService keeper = Executors.newSingleThreadScheduledExecutor(r -> {
        Thread t = new Thread(r, "upload-visibility");
        t.setDaemon(true);
        return t;
    });
    ...
    public QueueWorker(SqsClient sqs, String queueUrl, UploadProcessor processor, Duration visibilityTimeout,
                       Duration pollWait) {
        this.sqs = sqs;
    ...
    public void start() {
        running = true;
        loop = Thread.ofPlatform().name("upload-queue").start(this::loop);
        log.info("upload.worker_started queue={}", queueUrl);
    }
    ...
    private void loop() {
        while (running) {
            try {
                ...
                log.warn("upload.receive_failed err={}", e.getMessage());
                sleep(Duration.ofSeconds(5));
            } catch (RuntimeException e) {
                log.error("upload.loop_error", e);
                sleep(Duration.ofSeconds(5));
            }
        }
    }
    ...
    private void handle(Message message) {
        ...
        log.info("upload.message_done messageId={} receiveCount={} disposition={}", message.messageId(),
        ...
        try {
            ...
            // 지우기에 실패해도 다음 수신의 start가 proceed:false로 치운다.
            log.warn("upload.disposition_failed messageId={} disposition={} err={}", message.messageId(),
    ...
    private void extend(String receipt) {
        try {
            ...
            log.warn("upload.extend_failed err={}", e.getMessage());
        }
    }
